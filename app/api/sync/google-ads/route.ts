...
import { syncGoogleAdsCliente, syncGoogleAdsTodosClientes } from "@/lib/sync/googleAdsApiSync";
...
async function runSync(
  ...
  try {
    if (clienteId) {
      const result = await syncGoogleAdsCliente(clienteId, { dateFrom, dateTo });
...
export async function POST(request: NextRequest) {
  ...
  const clienteId = (body?.clienteId ?? request.nextUrl.searchParams.get("clienteId")) as
    | string
    | undefined;
  const dateFrom = (body?.dateFrom ?? request.nextUrl.searchParams.get("dateFrom")) as
    | string
    | undefined;
  const dateTo = (body?.dateTo ?? request.nextUrl.searchParams.get("dateTo")) as
    | string
    | undefined;
